/*==================================================================
 ...
 PROPÓSITO
   ...
   `mpitb`. Replica exactamente los resultados de H y M0 producidos
 ...
 MÉTODO ALKIRE-FOSTER — IMPLEMENTACIÓN PASO A PASO
   ...
   5. Medidas de resumen con diseño muestral complejo (svy):
        H  = media ponderada de poor_multi  (headcount ratio)
        M0 = media ponderada de c_censored  (= H * A, el IPM ajustado)

 INPUTS ESPERADOS
   - "${gdData}/Data Clean $MPM/DataDeprivations${MPM}.dta"
     (generado por 02_privaciones.do)
   - Globals: $MPM, $gdData (definidos en 00_maestro.do)

 OUTPUTS GENERADOS
   - "${gdStata}/${MPM}_results.dta"
     Resultados de H y M0 por nivel de agregación (nat/area/prov/
     cities), con las mismas columnas que el output de mpitb:
     measure, loa, subg, k, b, se.
     (Mismo path que 03_calculo_mpm_mpitb.do; compatible con
     04_exportar_figuras.do.)
   - "${gdData}/${MPM}_results_microdata.dta"
     Microdato a nivel de hogar con c_equal y poor_multi
     (compatible con 04_exportar_figuras.do).

 NOTA DE VALIDACIÓN
   Si ya existe "${gdStata}/${MPM}_results.dta" al inicio (producido
   por 03_calculo_mpm_mpitb.do), se guarda en un tempfile y al
   final se comparan H y M0 en todos los niveles de agregación.
==================================================================*/
...
keep hhid provincia cod_provincia cod_CV_CP age hhsize weight_hh         ///
     pcexp_ppp educat7 asistencia_escolar electricity imp_san_rec         ///
     imp_wat_rec dep_educ_com educ_enr_size dep_educ_enr dep_infra_elec  ///
     dep_infra_imps dep_infra_impw dep_poor1 quintile ruralprov cities
...
* Identificación de pobreza multidimensional: k = 33% (= 1/3)
...
*------------------------------------------------------------------
* 3. ESTIMACIÓN DE H y M0 CON DISEÑO MUESTRAL COMPLEJO
...
foreach a of local areas {
    if strpos("`area_type'", "str") > 0 { // to detect string vs numeric values  for which the conditional changes 
        quietly svy, subpop(if area == "`a'"): mean poor_multi c_censored
    }
    else {  
        quietly svy, subpop(if area == `a'): mean poor_multi c_censored
    }
...
foreach p of local provs {
    if strpos("`prov_type'", "str") > 0 { // to detect string vs numeric values  for which the conditional changes 
        quietly svy, subpop(if prov == "`p'"): mean poor_multi c_censored
    }
    else {
        quietly svy, subpop(if prov == `p'): mean poor_multi c_censored
    }
...
di ""
di "=== H y M0 nacionales (k=33%) — implementación manual ==="
...
keep hhid provincia cod_provincia cod_CV_CP hhsize weight_hh              ///
     electricity imp_san_rec imp_wat_rec dep_educ_com educ_enr_size       ///
     dep_educ_enr dep_infra_elec dep_infra_imps dep_infra_impw dep_poor1  ///
     quintile ruralprov cities pcexp_ppp educat7 asistencia_escolar
